...
@section('admin-page-title', 'Poetry')
...
@section('admin-main-section')
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="page-title">Manage Poetry</h1>
            @if (auth()->user()->user_role == 1)
                <button class="btn btn-primary off-canvas" type="button" data-bs-toggle="offcanvas"
                    data-bs-target="#offcanvasRight" aria-controls="offcanvasRight">Add Poetry</button>
            @endif
        </div>
    </div>
    <!-- PAGE-HEADER END -->
                            ...
                            <tbody>
                                @foreach ($poetrys as $poetry)
                                    <tr>
                                        ...
                                        <td>{{ \Illuminate\Support\Str::limit($poetry->description, 20) }}</td>
                                        <td class="text-center">
                                            <x-buttons.action-pill-button iconClass="fa fa-eye" iconColor="secondary"
                                                href="{{ route('admin.poetrys.show', $poetry->id) }}" />

                                            {{-- @if (auth()->user()->user_role != 3)
                                                <x-buttons.action-pill-button
                                                    href="{{ route('admin.poetrys.edit', $poetry->id) }}"
                                                    iconClass="fa fa-pencil" iconColor="warning" />
                                            @endif --}}

                                            @if (auth()->user()->user_role == 1)
                                                @if (auth()->user()->user_role == 1)
                                                    <form action="{{ route('admin.poetrys.destroy', $poetry->id) }}"
    ...
    <x-modal.right-offcanvas title="Add New Team" id="offcanvasRight" action="{{ route('admin.poetrys.store') }}"
        method="POST">
        ...
        <x-inputs.input-field label="Poetry" name="description" />
